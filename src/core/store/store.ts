...
export const [modelsCatalogStatus, setModelsCatalogStatus] = createSignal<
  'idle' | 'loading' | 'ready' | 'failed'
>('idle');
/** 当前 catalog 来源标签 */
export const [modelsCatalogSource, setModelsCatalogSource] =
  createSignal<CatalogSourceTag>('empty');
...
export const [modelsCatalogGeneratedAt, setModelsCatalogGeneratedAt] = createSignal<string | null>(
  null,
);
 ...
 */
export const [profileModelOverrides, setProfileModelOverrides] = createSignal<
...
export const [customSubagentProfiles, setCustomSubagentProfiles] = createSignal<
...
export const [mcpServerStatus, setMcpServerStatus] = createSignal<
 ...
 */
export const startMcpServerAndRefresh = async (
  id: string,
  ...
  try {
    const tools = await invoke<ToolSpec[]>('start_mcp_server', {
      id,
      projectId: projectId ?? null,
    });
    setMcpServerStatus((prev) => ({
 ...
 * 消费后立即清空，避免重复触发。
 */
export const [pendingRenameRequest, setPendingRenameRequest] = createSignal<{
  asstId: string;
  topicId: string;
} | null>(null);
