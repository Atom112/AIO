...
function profileLabel(profileId?: string): string {
  switch (profileId) {
    case 'explorer':
      return t('agent.profile.explorer');
    case 'coder':
      return t('agent.profile.coder');
    case 'general':
      return t('agent.profile.general');
    case 'requirements':
      return t('agent.profile.requirements');
    default:
      return t('agent.profile.subagent');
  }
}
...
function stepTypeLabel(type: SubagentStep['type']): string {
  switch (type) {
    case 'thinking':
      return t('agent.sub.step.thinking');
    case 'tool_call':
      return t('agent.sub.step.toolCall');
    case 'content':
      return t('agent.sub.step.content');
  }
}
      ...
      style={
        {
          ...
          '--sa-bg': profileStyle().bgColor,
          '--sa-border': profileStyle().borderColor,
          background: profileStyle().bgColor,
          'border-color': profileStyle().borderColor,
        } as any
      }
      classList={{
      ...
      <Show when={props.expanded}>
        <div class="px-2 pb-2" style={{ 'max-height': '400px', 'overflow-y': 'auto' }}>
          ...
          <Show when={steps().length > 0}>
            <div
              class="flex flex-col gap-0.5 border-l-2 ml-2 pl-2"
              style={{ 'border-color': 'var(--sa-border)' }}
            >
              <For each={steps()}>{(subStep) => <SubStepRow step={subStep} />}</For>
            </div>
          </Show>
          ...
          <Show when={steps().length === 0 && props.step.status === 'running'}>
            <div class="text-[11px] text-white/25 italic px-2 py-2">{t('agent.sub.working')}</div>
          </Show>

          {/* 错误状态 */}
          <Show when={props.step.status === 'error'}>
            <div class="text-[12px] text-red-400/60 px-2 py-2">{t('agent.sub.error')}</div>
          </Show>
        </div>
      </Show>
    </div>
