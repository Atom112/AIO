...
const ModelBreakdown: Component<Props> = (props) => {
    ...
    const items = createMemo(() => {
        const cat = catalog();
        const list = props.byModel.map((m, i) => {
            const total = m.inputTokens + m.outputTokens;
            const meta = cat?.models.find(md => md.id === m.modelId);
            ...
            <For each={items()}>
                {(item, i) => (
                    ...
                    >
                        ...
                        {/* 次级行：input/output + 费用 */}
                        <div class="flex items-center gap-3 mt-1.5 text-[10px] font-mono" style="color: rgba(255,255,255,0.25);">
                            <span class="flex-1" />
